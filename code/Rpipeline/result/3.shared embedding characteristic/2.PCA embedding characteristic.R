...
#######
pdf(paste0(res_path,'phen_score_t_SNE.pdf'))
for( i in 1:5){
...
pdf(paste0(res_path,'/PCA_Immune_features_t_SNE.pdf'))
for( i in 1:4){
...
library(ggsci)
pdf(paste0(res_path,'/PCA_cancer_type_t_SNE.pdf'))
p=ggplot(tsne_result,aes(tSNE1,tSNE2)) +
...
spearman_corr_res <- lapply(1:ncol(PCA_outputs), function(i){
...
save(spearman_corr_rho, spearman_corr_pvalue, spearman_corr_adj_pvalue, 
     file=paste0(res_path,"/PCA_nodes_output_corr_imm.RData" ))
...
pheatmap(plot.data,cluster_row = FALSE,cluster_cols = TRUE, 
         ...
         filename = paste0(res_path,'PCA_imm_corr_heatmap.pdf'),
         width=15,
         height=2.5
) 
